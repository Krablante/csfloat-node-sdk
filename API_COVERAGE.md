...
88. `sticker_option=skins|packages` is bundle-observed and accepted by the API, but top-page live probes on 2026-03-08 returned the same leading ids for both values; treat `sticker_option` as only weakly mapped for now and keep it docs-only until a clearer semantic difference is proven
89. direct authenticated live probes on 2026-03-08 also confirmed that `GET /me/watchlist?stickers=[...]` reuses the same JSON sticker filter contract: `stickers=[{"i":3}]` returned the matching watched `Souvenir M4A1-S | VariCamo (Field-Tested)` row with an applied `{ stickerId: 3, slot: 3 }` payload; the same account returned `200` but zero rows for `keychains=[{"i":1}]`, so watchlist keychain semantics remain only lightly mapped for now
